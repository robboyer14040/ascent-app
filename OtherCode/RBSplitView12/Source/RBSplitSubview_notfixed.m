...
// Loop while the button is down.
		while ((theEvent = [NSApp nextEventMatchingMask:NSEventMaskLeftMouseDown|NSEventMaskLeftMouseDragged|NSEventMaskLeftMouseUp
                                              untilDate:[NSDate distantFuture]
                                                 inMode:NSEventTrackingRunLoopMode dequeue:YES]) && ([theEvent type]!=NSEventMaskLeftMouseUp)) {
// Set up a local autorelease pool for the loop to prevent buildup of temporary objects.
			NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
			// rcb NSDisableScreenUpdates();
...
// is visible right here! So we fake it and move the window as intended. Mwahahaha!
		/// rcb where =  [window convertBaseToScreen:where];
        where =  [window convertPointToScreen:where];
		NSPoint origin = [window frame].origin;
// Now we loop handling mouse events until we get a mouse up event.
		while ((theEvent = [NSApp nextEventMatchingMask:NSEventMaskLeftMouseDown|NSEventMaskLeftMouseDragged|NSEventMaskLeftMouseUp untilDate:[NSDate distantFuture] inMode:NSEventTrackingRunLoopMode dequeue:YES])&&([theEvent type]!=NSEventMaskLeftMouseUp)) {
// Set up a local autorelease pool for the loop to prevent buildup of temporary objects.
			NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
			/// rcb NSPoint now = [window convertBaseToScreen:[theEvent locationInWindow]];
