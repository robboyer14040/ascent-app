...
static void _AcceptCallBack(CFSocketRef socket, CFSocketCallBackType type, CFDataRef address, const void* data, void* info)
{
	...
	TCPService*				service = (__bridge TCPService*)info;
...
{
    CFSocketContext				socketCtxt = {0, reinterpret_cast<TCPService*>(self), NULL, NULL, NULL};
	int							yes = 1;
...
- (BOOL) enableBonjourWithDomain:(NSString*)domain applicationProtocol:(NSString*)protocol name:(NSString*)name
{
	...
	[_netService scheduleInRunLoop:_runLoop forMode:NSDefaultRunLoopMode];
	[_netService publish];
...
- (void) disableBonjour
{
	if(_netService) {
		...
		[_netService removeFromRunLoop:_runLoop forMode:NSDefaultRunLoopMode];
		_netService = nil;
	}
}
...
{
	return [NSString stringWithFormat:@"<%@ = 0x%08lX | running = %i | local address = %@>", [self class], (long)self, [self isRunning], SockaddrToString(_localAddress)];
}
