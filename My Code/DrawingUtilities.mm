...
NSString*  buildText(float value, float max, BOOL isTime)
{
   ...
   if (isTime)
   {
      ...
      s = [NSString stringWithFormat:@"%s%2.2d:%2.2d", isNeg ? "-" : "", val/60, val%60 ];
   }
   else 
   {
      NSString* format;
                     ...
                     float xClipAfter)
{
	...
	if (0.0 != diff)
	{
		...
		if ((axis == kVerticalLeft) || (axis == kVerticalRight))
		{
			...
			BOOL skipEven = (numTickPoints & 0x01) == 1;
			BOOL skip = (numTickPoints >= 10) && skipEven;
			NSColor* color = (NSColor*)[tickFontAttrs objectForKey:NSForegroundColorAttributeName];
            [color set];
			...
			for (i=0; i<numTickPoints+1; i++)
			{
				...
				BOOL drawText = ((skip == NO) || (i==0) || (calt == 0.0));
		...
		else if (axis == kHorizontal)
		{
			...
			float dx = w/numTickPoints;
			float diffdx = diff/numTickPoints;
			float addXoff = (isTime) ? -6.0 : -5.0;
...
CGSize GetCGTextSize(CGContextRef context, NSString* s)
{
	...
	CGContextShowText(context, [s UTF8String], [s length]);
	CGPoint pp = CGContextGetTextPosition(context);
	CGSize size;
	size.height = pp.y;
	size.width = pp.x;
