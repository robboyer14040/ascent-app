...
-(GarminFIT*)initWithFileURL:(NSURL*)url
{
	...
	if (self)
	{
       ...
       NSCalendar *cal = [[NSCalendar alloc] initWithCalendarIdentifier:NSCalendarIdentifierGregorian];
		...
		[comps setMinute:0];
		[comps setSecond:0];
		refDate = [cal dateFromComponents:comps];
			...
			switch (convert_return)
			{
				case FIT_CONVERT_MESSAGE_AVAILABLE:
				{
					...
					switch(mesg_num)
                    {
                        ...
                        {
                            ///const FIT_DEVELOPER_DATA_ID_MESG *devMesg = (FIT_DEVELOPER_DATA_ID_MESG*) mesg;
                            break;
                        }
                            
                        case FIT_MESG_NUM_EXERCISE_TITLE:
                        {
                            ///const FIT_EXERCISE_TITLE_MESG *titleMesg = (FIT_EXERCISE_TITLE_MESG*) mesg;
                            break;
                        }
                        case FIT_MESG_NUM_SEGMENT_ID:
                        {
                            ///const FIT_SEGMENT_ID_MESG *segmentMesg = (FIT_SEGMENT_ID_MESG*) mesg;
                            break;
                        }
                           
                        case FIT_MESG_NUM_FIELD_DESCRIPTION:
                        {
                            ///const FIT_FIELD_DESCRIPTION_MESG *fdMesg = (FIT_FIELD_DESCRIPTION_MESG*) mesg;
                            break;
                        }
						...
						case FIT_MESG_NUM_LAP:
						{
                            ...
                            {
                                distance = ((float)lap->total_distance)/100.0;		// stored as 100 * m
                            }
							float max_speed = 0.0;
							...
							Lap* ascentLap = [[Lap alloc] initWithGPSData:numLaps++
													startTimeSecsSince1970:(time_t)startSecs 
															 ...
															 totalDistance:distance     // NOTE: specified as METERS!!!
																  maxSpeed:max_speed
																  beginLat:blat 
																  beginLon:blon 
																	endLat:elat 
							...
							[ascentLap setDeviceTotalTime:lap->total_timer_time/1000.0];	// excludes pauses
                            NSTimeInterval ttt = [track deviceTotalTime] + (lap->total_timer_time/1000.0);
                            [track setDeviceTotalTime:ttt];         // excludes pauses
                            
                            
							[lapArray addObject:ascentLap];
                            if (distance <= 0.0) nonMovingLap = YES;
							break;
						}
						...
						case FIT_MESG_NUM_RECORD:
						{
							...
							if (activeTimeDelta >= 0.0)
							{
								...
								if (hasSpeed)
								{
									speed = MetersToMiles(((float)record->speed*3600.0)/(1000.0));	// stored as 1000 * m/s
                                    lastSpeed = speed;
 								}
									 ...
									 (record->compressed_speed_distance[2] != FIT_BYTE_INVALID)))
								{
									...
									speed = MetersToMiles((speed100*3600.0)/100.0f);
                                    lastSpeed = speed;
								...
								{
									heartrate = lastHeartRate = (int)record->heart_rate;
								}
                                else
                                {
								...
								{
									cadence = lastCadence = (int)record->cadence;
								}
                                else
                                {
                                    cadence = lastCadence;
						...
						case FIT_MESG_NUM_EVENT:
						{
							...
							else if (!nonMovingLap && (event->event == FIT_EVENT_TIMER) &&
