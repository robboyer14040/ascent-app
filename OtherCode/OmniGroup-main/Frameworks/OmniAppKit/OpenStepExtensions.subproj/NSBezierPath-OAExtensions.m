...
- (NSPoint)currentpointForSegment:(NSInteger)i
{
    ...
    switch(element) {
        case NSBezierPathElementQuadraticCurveTo: {
            // Normalize quadratic (P0,Q,P2) → cubic (P0,C1,C2,P2); fall through to CurveTo
            NSPoint q = points[0];
            NSPoint p2 = points[1];
            NSPoint p0 = [self currentPoint];
            NSPoint c1 = NSMakePoint(p0.x + (2.0/3.0)*(q.x - p0.x),
                                     p0.y + (2.0/3.0)*(q.y - p0.y));
            NSPoint c2 = NSMakePoint(p2.x + (2.0/3.0)*(q.x - p2.x),
                                     p2.y + (2.0/3.0)*(q.y - p2.y));
            points[0] = c1;
            points[1] = c2;
            points[2] = p2;
            /* fall through to NSBezierPathElementCurveTo */
        }
...
- (NSCountedSet *)countedSetOfEncodedStrokeSegments;
{
    ...
    for(elementIndex=0; elementIndex<elementCount; elementIndex++) {
        ...
        switch(element) {
            case NSBezierPathElementQuadraticCurveTo:
                /* Not handled in this context; keeping behavior unchanged */
                break;
            case NSBezierPathElementMoveTo:
    ...
    for(i=1;i<count;i++) {
        ...
        switch(element) {
            ...
            case NSBezierPathElementClosePath:
                ...
                break;
            case NSBezierPathElementQuadraticCurveTo: {
                // Approximate quadratic as straight segment; fall through to LineTo
                points[0] = points[1];
                /* fall through to NSBezierPathElementLineTo */
            }
            case NSBezierPathElementLineTo:
...
- (BOOL)firstIntersectionWithLine:(OABezierPathIntersection *)result lineStart:(NSPoint)lineStart lineEnd:(NSPoint)lineEnd;
{
    ...
    while(nextSubpathElement(iter)) {
        ...
        switch(iter.what) {
            ...
            case NSBezierPathElementQuadraticCurveTo: {
                // Normalize quadratic (P0,Q,P2) → cubic (P0,C1,C2,P2); fall through to CurveTo
                NSPoint q = iter->points[0];
                NSPoint p2 = iter->points[1];
                NSPoint p0 = [self currentPoint];
                NSPoint c1 = NSMakePoint(p0.x + (2.0/3.0)*(q.x - p0.x),
                                         p0.y + (2.0/3.0)*(q.y - p0.y));
                NSPoint c2 = NSMakePoint(p2.x + (2.0/3.0)*(q.x - p2.x),
...
static void parameterizeSubpathElement(_OASubpathWalkingState *st, NSPoint elementCoefficients[4])
{
    switch(st.what) {
        ...
        case NSBezierPathElementLineTo:
            ...
            break;
        case NSBezierPathElementQuadraticCurveTo: {
            // Convert quadratic (P0=st->points[0], Q=st->points[1], P2=st->points[2]) to cubic and parameterize
            NSPoint p0 = st->points[0];
            NSPoint q  = st->points[1];
            NSPoint p2 = st->points[2];
            NSPoint c1 = NSMakePoint(p0.x + (2.0/3.0)*(q.x - p0.x),
                                     p0.y + (2.0/3.0)*(q.y - p0.y));
            NSPoint c2 = NSMakePoint(p2.x + (2.0/3.0)*(q.x - p2.x),
                                     p2.y + (2.0/3.0)*(q.y - p2.y));
            _OAParameterizeCurve(elementCoefficients, p0, p2, c1, c2);
            break;
        }
case NSBezierPathElementCurveTo:
            ...
            if (self == other && selfIter.currentElt > otherIter.currentElt) {
                ...
                case NSBezierPathElementLineTo:
                    switch(otherIter.what) {
                        ...
                        case NSBezierPathElementQuadraticCurveTo: {
                            // Normalize quadratic (P0,Q,P2) → cubic (P0,C1,C2,P2); fall through to CurveTo
                            NSPoint q = otherIter->points[0];
                            NSPoint p2 = otherIter->points[1];
                            NSPoint p0 = [self currentPoint];
                            NSPoint c1 = NSMakePoint(p0.x + (2.0/3.0)*(q.x - p0.x),
                                                     p0.y + (2.0/3.0)*(q.y - p0.y));
                            NSPoint c2 = NSMakePoint(p2.x + (2.0/3.0)*(q.x - p2.x),
                                                     p2.y + (2.0/3.0)*(q.y - p2.y));
                            otherIter->points[0] = c1;
                            otherIter->points[1] = c2;
                            otherIter->points[2] = p2;
                            /* fall through to NSBezierPathElementCurveTo */
                        }
                        case NSBezierPathElementCurveTo:
...
static double subpathElementLength(_OASubpathWalkingState *iter, double errorBudget)
{
    switch(iter.what) {
        ...
        case NSBezierPathElementQuadraticCurveTo: {
            // Convert quadratic (P0=iter->points[0], Q=iter->points[1], P2=iter->points[2]) to cubic and fall through
            NSPoint p0 = iter->points[0];
            NSPoint q  = iter->points[1];
            NSPoint p2 = iter->points[2];
            NSPoint c1 = NSMakePoint(p0.x + (2.0/3.0)*(q.x - p0.x),
                                     p0.y + (2.0/3.0)*(q.y - p0.y));
            NSPoint c2 = NSMakePoint(p2.x + (2.0/3.0)*(q.x - p2.x),
                                     p2.y + (2.0/3.0)*(q.y - p2.y));
        ...
        if (cursor.currentElt == seg) {
            if (parameter > 0) {
                switch(cursor.what) {
                    ...
                    case NSBezierPathElementQuadraticCurveTo: {
                        // Approximate quadratic as straight segment; fall through to LineTo
                        cursor->points[0] = cursor->points[1];
                        /* fall through to NSBezierPathElementLineTo */
                    }
                    case NSBezierPathElementLineTo:
...
{    
    ...
    switch(cursor.what) {
        ...
        case NSBezierPathElementQuadraticCurveTo: {
            // Normalize quadratic (P0,Q,P2) → cubic (P0,C1,C2,P2); fall through to CurveTo
            NSPoint q = cursor->points[0];
            NSPoint p2 = cursor->points[1];
            NSPoint p0 = [self currentPoint];
            NSPoint c1 = NSMakePoint(p0.x + (2.0/3.0)*(q.x - p0.x),
                                     p0.y + (2.0/3.0)*(q.y - p0.y));
            NSPoint c2 = NSMakePoint(p2.x + (2.0/3.0)*(q.x - p2.x),
    ...
    for(i=0;i<count;i++) {
        ...
        switch(element) {
            ...
            case NSBezierPathElementQuadraticCurveTo: {
                ...
                NSPoint q = points[0];
                NSPoint p2 = points[1];
                NSPoint p0 = [self currentpointForSegment:i];
                NSPoint c1 = NSMakePoint(p0.x + (2.0/3.0)*(q.x - p0.x),
                                         p0.y + (2.0/3.0)*(q.y - p0.y));
                NSPoint c2 = NSMakePoint(p2.x + (2.0/3.0)*(q.x - p2.x),
                                         p2.y + (2.0/3.0)*(q.y - p2.y));
    ...
    for(elementIndex=0; elementIndex<elementCount; elementIndex++) {
        ...
        switch (element) {
            ...
            case NSBezierPathElementQuadraticCurveTo: {
                ...
                NSPoint q = points[0];
                NSPoint p2 = points[1];
                NSPoint p0 = [self currentpointForSegment:elementIndex];
                NSPoint c1 = NSMakePoint(p0.x + (2.0/3.0)*(q.x - p0.x),
                                         p0.y + (2.0/3.0)*(q.y - p0.y));
                NSPoint c2 = NSMakePoint(p2.x + (2.0/3.0)*(q.x - p2.x),
                                         p2.y + (2.0/3.0)*(q.y - p2.y));
...
void repositionSubpathWalkingState(_OASubpathWalkingState *s, NSInteger toIndex)
{
    ...
    switch(previousElt) {
        ...
        case NSBezierPathElementMoveTo:
            break;
        case NSBezierPathElementQuadraticCurveTo:
            s->points[0] = s->points[1];  // update currentpoint to end point
            break;
        case NSBezierPathElementCurveTo:
