...
- (NSCell *)cell
{
	...
	{
		[cell setSegmentStyle:FlatSegmentedCellStyle];
    }
	else
	{
...
- (void)resetBrowserTrackRow:(Track*)trk extend:(BOOL)ext
{
	...
	int row = [trackTableView rowForItem:biOfTrack];
	if (row >= 0)
	{
	...
	for (int i=0; i<numRows; i++)
	{
		...
		if ([tbi track] == track)
		{
			...
			while (TrackBrowserItem* parentItem = [tbi parentItem])
			{
				...
				int sr = [trackTableView rowForItem:parentItem];
...
-(void)expandCapturedExpandedItemsByName
{
    if (expandedItemNames)
    {
        int count = [expandedItemNames count];
...
-(void)restoreSelectedRowsViewToggle:(NSSet*)selectedSet
{
	...
	for (id item in localSet)
	{
		...
		int idx = [trackTableView rowForItem:tbi];
		if (idx != -1)
		{
			[is addIndex:idx];
		}
...
-(void) buildContextualMenu
{
	...
	[[cm addItemWithTitle:@"Upload to Mobile"
				   action:@selector(setUploadToMobile:)
	...
	[mi setKeyEquivalentModifierMask:NSControlKeyMask];
...
-(void) restoreSelectedRowsAfterExpand
{
	...
	for (id item in localSet)
	{
		int idx = [trackTableView rowForItem:item];
		if (idx != -1)
		{
			[is addIndex:idx];
		}
...
static tEventInfo sEventInfo[] = 
{
	...
	{ "Temperature",	"max", "ûF",		"ûC",		kST_Temperature,	kMax, &FahrenheightToCelsius, kUseNumberFormatter },
	{ "Temperature",	"min", "ûF",		"ûC",		kST_Temperature,	kMin, &FahrenheightToCelsius, kUseNumberFormatter },
};
									 ...
									 row:(int)rowIndex
{
	...
	if (currentlySelectedLap || currentlySelectedTrack)
	{
		...
		else if ([ident isEqualToString:@"Units"])
		{
			...
			{
				value = [NSString stringWithCString:eventInfo.unitsStatute
										   encoding:NSUTF8StringEncoding];
			}
			else
			{
				value = [NSString stringWithCString:eventInfo.unitsMetric
										   encoding:NSUTF8StringEncoding];
			}
		}
		else
...
- (BOOL)outlineView:(NSOutlineView *)outlineView writeItems:(NSArray *)items toPasteboard:(NSPasteboard *)pboard
{
	...
	for (TrackBrowserItem* bi in items)
	{
		...
		{
			int idx = [trackArray indexOfObjectIdenticalTo:[bi track]];
			if (idx != NSNotFound)
			{
...
{
	return [NSString stringWithFormat:@"%s/Desktop", NSHomeDirectory()];
}
...
- (void)controlTextDidBeginEditing:(NSNotification *)aNotification
{
	...
	if (obj == notesTextView)
	{
		//[obj setAlignment:NSLeftTextAlignment];
	}
}
...
{
	[self doToggleCalendarAndBrowser:[sender selectedSegment]];
}


- (IBAction)setUploadToMobile:(id)sender
{
	BOOL up = [currentlySelectedTrack uploadToMobile];
	[currentlySelectedTrack setUploadToMobile:!up];
}
	...
	for (i=0; i<numTracks; i++)
	{
		...
		if ([self passesSearchCriteria:track])
		{
			...
			if (comps != nil)
			{
				...
				int ws = [comps day] - dayOfWeek + weekStartDay;    // weekStartDay is 0 for sunday
				...
				if ((viewType == kViewTypeCurrent) || /*(viewType == kViewTypeYears) || (viewType == kViewTypeMonths) || */ (viewType == kViewTypeWeeks)  )
				{
					...
					if (viewType != kViewTypeWeeks) weekKey = [weekKey stringByAppendingFormat:@"_%d_%d", year, month];
