...
- (void)awakeFromNib
{
    ...
    if (_outlineView != nil) {
        ...
        _outlineView.target       = self;                           // who receives actions
        _outlineView.doubleAction = @selector(_didDoubleClick);
     }
...
- (void)viewDidLoad {
    [[NSNotificationCenter defaultCenter] addObserverForName:TrackArrayChangedNotification
                                                      object:nil
...
- (void)buildBrowser:(BOOL)expandLastItem
{
    ...
    for (NSUInteger i = 0; i < numTracks; i++) {
        ...
        if ((_viewType == kViewTypeNormal) || (_viewType == kViewTypeYears)) {
        ...
        if ((_viewType == kViewTypeNormal) || (_viewType == kViewTypeYears) || (_viewType == kViewTypeMonths)) {
