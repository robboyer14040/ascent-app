...
- (BOOL)acceptsFirstMouse:(NSEvent*)theEvent {
	/// rcb return ([theEvent modifierFlags]&NSCommandKeyMask)==0;
...
// Now we loop handling mouse events until we get a mouse up event, while showing the drag cursor.
				...
				while ((theEvent = [NSApp nextEventMatchingMask:NSEventMaskLeftMouseDown|NSEventTypeLeftMouseUp|NSEventMaskLeftMouseDragged untilDate:[NSDate distantFuture] inMode:NSEventTrackingRunLoopMode dequeue:YES])&&([theEvent type]!=NSEventTypeLeftMouseUp)) {
...
// Draw the image if the delegate returned a non-empty rect.
	if (!NSIsEmptyRect(dorect)) {
		[anImage drawInRect:dorect fromRect:imrect operation:NSCompositingOperationSourceOver fraction:1.0];
	}
}
