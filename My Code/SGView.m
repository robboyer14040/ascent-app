...
- (id)initWithFrame:(NSRect)frameRect
{
   ...
   dataDict  = [NSMutableDictionary dictionaryWithCapacity:52];
   [dataDict retain];
   ...
   if (0.0 < [tid maxValue])
   {
      ...
      for (i=0; i<kNumPlotTypes; i++)
      {
         ...
         if ([ti enabled])
         {
            ...
            if ([ti isTimeValue]) 
            {
                [ti setNumTicks:AdjustTimeRuler(20, 0,  [ti maxValue], &incr)];
            }
            else
            {
...
- (NSArray *)typeInfoArray {
   ...
   return [[typeInfoArray retain] autorelease];
}
